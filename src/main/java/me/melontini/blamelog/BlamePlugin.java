...
public class BlamePlugin implements IMixinConfigPlugin {
  ...
  static {
    if (InstrumentationAccess.canInstrument()) {
      ...
      if (AbstractLogger.class.getClassLoader() != BlamePlugin.class.getClassLoader()) {
        ...
        LOGGER.warn(
            "[BlameLog] This means you're probably running Quilt or Connector. Be ware of issues!");

        try (InputStream stream = BlamePlugin.class
            .getClassLoader()
            .getResourceAsStream("me/melontini/blamelog/BlameUtil.class")) {
          byte[] bytes =
              MakeSure.notNull(stream, "Can't access BlameUtil.class").readAllBytes();

          ClassLoader a = AbstractLogger.class.getClassLoader();
          // Thanks Su5eD for the fix.
          // https://github.com/Sinytra/Connector/discussions/12#discussioncomment-6790140
          InstrumentationAccess.addReads(AbstractLogger.class.getModule(), a.getUnnamedModule());

          blameUtil = tryDefineClass(
              a, "me.melontini.blamelog.BlameUtil", bytes, BlameUtil.class.getProtectionDomain());
        } catch (IOException
      ...
      AtomicInteger integer =
          new AtomicInteger(); // Keeping track of the number of methods we've retransformed. Not
      // really necessary, but it's nice to know.
      try {
        InstrumentationAccess.retransform(
            node -> LogPatcher.patch(node, integer),
  ...
  private static Class<?> tryDefineClass(
      ClassLoader a, String name, byte[] bytes, ProtectionDomain domain)
    ...
    try {
      ...
      if (!defineClass.canAccess(null)) {
        if (InstrumentationAccess.get().isModifiableModule(javaBase)) {
          ...
          InstrumentationAccess.addOpens(
              javaBase, Map.of("java.lang", Set.of(BlamePlugin.class.getModule())));
