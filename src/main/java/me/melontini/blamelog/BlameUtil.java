...
public class BlameUtil {
  ...
  private static final Map<String, BiFunction<StackWalker.StackFrame, MixinMerged, String>>
      patterns = Map.of(
          "{class}", (frame, mixin) -> mixin == null ? frame.getClassName() : mixin.mixin(),
          "{method}", (frame, mixin) -> frame.getMethodName(),
          "{simpleClass}",
              (frame, mixin) -> mixin == null
                  ? simpleClassName(frame.getDeclaringClass())
                  : simpleClassName(mixin.mixin()),
          "{methodParams}",
              (frame, mixin) -> {
          ...
          "{methodReturnType}",
              (frame, mixin) -> simpleClassName(frame.getMethodType().returnType()));
  ...
  static {
    ...
    String[] classEnds =
        new String[] {"Logger", "Log", "LogHelper", "LoggerAdapterAbstract", "Logging"};
    base.add(frame -> {
  ...
  public static String getMessage(String msg) {
    ...
    if (frame.getClassName().startsWith("net.minecraft")
        && !StringUtils.equalsAny(methodName, "<init>", "<clinit>")) {
    ...
    for (Map.Entry<String, BiFunction<StackWalker.StackFrame, MixinMerged, String>> entry :
        patterns.entrySet()) {
